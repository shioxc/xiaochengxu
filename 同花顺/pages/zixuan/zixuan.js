...
Page({
  data: {
    stockList: [],
    filteredStockList: [], // 用于存储筛选后的股票列表
  },
  ...
  navigateToStockDetail(e) {
    ...
    const jys = e.currentTarget.dataset.jys.toLowerCase()
    console.log("jys",jys)
    wx.navigateTo({
  ...
  handleSearchInput(e) {
    ...
    if (keyword === '') {
      this.setData({
        filteredStockList: this.data.stockList
      });
    } else {
      const filteredList = this.data.stockList.filter(stock => {
        const stockName = stock.mc.toLowerCase();
        const stockCode = stock.dm.slice(0, 6).toLowerCase();
        return stockName.includes(keyword) || stockCode.includes(keyword);
      });
      const formattedList = filteredList.map(stock => {
        // 创建新对象，避免修改原始数据
        return {
          ...stock,
          dm: stock.dm.slice(0, 6) // 截取dm字段的前6个字符
        };
      });
      this.setData({
        filteredStockList: formattedList
      });
    }
  },
  /**
  ...
  onLoad(options) {
    ...
    wx.request({
      ...
      success: (res) => {
        let newStockList = res.data.slice(0, 100);
        ...
        Promise.all(requests).then(() => {
          ...
          this.setData({
            ...
            filteredStockList: newStockList // 初始时显示全部股票列表
          });
        }).catch((error) => {
